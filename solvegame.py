...
Statistics = list()
for i in range(0,4):
    Statistics.append(copy.deepcopy(simple_stats))
print('Smart Iterative deepening')
t0 = time.time()
SIDsolution_found, SIDnodes_expanded, SIDstats, SIDnodes_added, found_depth = Iterative_Deepening(start_Node,20)
t1 = time.time()
SIDrun_time = t1-t0
Statistics[0]['solution_found'] += SIDsolution_found
Statistics[0]['nodes_expanded'] += SIDnodes_expanded
Statistics[0]['nodes_added'] += SIDnodes_added
Statistics[0]['run_time'] += SIDrun_time
Statistics[0]['runs'] += 1
print('Smart A*')
t0 = time.time()
Asolution_found, Anodes_expanded, Astats, Anodes_added = Astar(start_Node)
t1 = time.time()
Arun_time = t1-t0
Statistics[1]['solution_found'] += Asolution_found
Statistics[1]['nodes_expanded'] += Anodes_expanded
Statistics[1]['nodes_added'] += Anodes_added
Statistics[1]['run_time'] += Arun_time
Statistics[1]['runs'] += 1
print('Smart BFS')
t0 = time.time()
SBsolution_found, SBnodes_expanded, SBstats, SBnodes_added = Breadth_First(start_Node, True)
t1 = time.time()
SBrun_time = t1-t0
Statistics[2]['solution_found'] += SBsolution_found
Statistics[2]['nodes_expanded'] += SBnodes_expanded
